class KafkaProducerTestSuite():
    ...
    def assertKafka(self, start_time, expected_count):
        ...
        actual_count = ( spark.read
                            ...
                            .option("startingOffsetsByTimestampStrategy", "latest") # if it doesnt find offset for certain time (startingTimestamp), it starts from the latest offset (topic -> partition -> offset)
                            .load()
                            .count()
                        )
